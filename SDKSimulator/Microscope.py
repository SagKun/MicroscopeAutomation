...
class Microscope:
    def __init__(self,config, connection_key):
        ...
        self.motor_current_vector = np.round(
            np.random.randn(3) / np.linalg.norm(np.random.randn(3)) * np.random.rand() * 3000).astype(int)
    ...
    def set_config(self, config):
        self.z_axis_min_step = config['z_axis_min_step']
        self.z_axis_upper_step_limit = config['z_axis_upper_step_limit']
        self.z_step_to_mm_ratio = config['z_step_to_mm_ratio']
        self.z_step_min_range = config['z_step_min_range']
        self.z_step_max_range = config['z_step_max_range']
